...
class MESHREPAIR_PT_Main(MESHREPAIR_PT_Base):
    ...
    def draw(self, context):
        ...
        # Operation buttons based on mode
        if props.operation_mode == 'CUSTOM':
            self.draw_custom_mode(col, props)
        else:
            self.draw_quick_mode(col, props)

    def draw_custom_mode(self, layout, props):
