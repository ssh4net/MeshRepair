...
void ThreadManager::configure_thread_counts() {
...
void ThreadManager::print_config() const {
    ...
    std::cout << "  Pipeline split: " << config_.detection_threads
              << " detection + " << config_.filling_threads << " filling\n\n";
}

void ThreadManager::enter_detection_phase() {
    // All threads for detection, none for filling
    detection_pool_.resize(config_.num_threads);
    filling_pool_.resize(0);

    if (config_.verbose) {
        std::cout << "[Threading] Detection phase: "
                  << config_.num_threads << " thread(s)\n";
    }
}

void ThreadManager::enter_pipeline_phase() {
