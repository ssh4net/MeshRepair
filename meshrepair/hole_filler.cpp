...
HoleStatistics
HoleFiller::fill_hole(const HoleInfo& hole)
{
    ...
    if (should_skip_hole(hole)) {
        if (options_.verbose) {
            std::cout << "    Skipping hole (exceeds size limits): " << hole.boundary_size << " boundary vertices\n";
        }
    ...
    try {
        ...
        if (triangulation_success) {
            ...
            if (options_.verbose) {
                std::cout << "    Filled: " << stats.num_faces_added << " faces, " << stats.num_vertices_added
                          << " vertices added";
                if (!fairing_success) {
...
MeshStatistics
HoleFiller::fill_all_holes(const std::vector<HoleInfo>& holes)
{
    ...
    for (size_t i = 0; i < holes.size(); ++i) {
        ...
        if (options_.verbose) {
            std::cout << "  Hole " << (i + 1) << "/" << holes.size() << " (" << holes[i].boundary_size
                      << " boundary vertices):\n";
        }
...
bool
HoleFiller::should_skip_hole(const HoleInfo& hole) const
{
...
double
HoleFiller::compute_mesh_bbox_diagonal() const
{
...
void
HoleFiller::report_progress(size_t current, size_t total, const std::string& message) const
{
    if (options_.show_progress && total > 0) {
        double percentage = (static_cast<double>(current) / total) * 100.0;
        std::cout << "  [" << current << "/" << total << " - " << static_cast<int>(percentage) << "%] " << message
                  << "\n";
    }
}
