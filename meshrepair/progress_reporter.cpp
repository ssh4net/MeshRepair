...
void ProgressReporter::start(size_t total_steps, const std::string& operation_name) {
    if (!enabled_) return;
...
void ProgressReporter::update(size_t current_step) {
    if (!enabled_ || total_steps_ == 0) return;
...
void ProgressReporter::finish() {
    ...
    std::cout << "[" << operation_name_ << "] Completed in "
...
void ProgressReporter::print_progress_bar(size_t current, size_t total, double elapsed_seconds) {
    ...
    std::cout << "] " << int(progress * 100.0) << "% "
              << "(" << current << "/" << total << ") "
              << std::fixed << std::setprecision(1) << elapsed_seconds << "s\r";
    std::cout.flush();
